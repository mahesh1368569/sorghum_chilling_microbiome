...
output_dir <- file.path(getwd(), "ITS_Network_Analysis")
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

asv <- read.csv("ASV_ITS_filt.csv", check.names = FALSE)
tax <- read.csv("ASV_ITStaxa_filt.csv", check.names = FALSE)

colnames(asv)[1] <- "Sample_ID"
colnames(tax)[1] <- "ASV"
tax <- tax %>% select(-X)
tax <- tax %>% mutate(across(c(Kingdom, Phylum, Class, Order, Family, Genus, Species), ~str_remove(.x, "^[a-z]__")))
...
ks_e_nodes <- tibble(name = sort(unique(c(ks_e_edges$from, ks_e_edges$to)))) %>% left_join(tax %>% select(ASV, Kingdom, Phylum, Class, Order, Family, Genus, Species), by = c("name" = "ASV"))
...
ggsave(file.path(output_dir, "Kansas_Early_ITS_network.pdf"), p_ks_e, width = 8, height = 7)
write.csv(ks_e_edges, file.path(output_dir, "Kansas_Early_ITS_edges.csv"), row.names = FALSE)
write.csv(as_tibble(as_data_frame(g_ks_e, what = "vertices")), file.path(output_dir, "Kansas_Early_ITS_nodes.csv"), row.names = FALSE)
...
ks_r_nodes <- tibble(name = sort(unique(c(ks_r_edges$from, ks_r_edges$to)))) %>% left_join(tax %>% select(ASV, Kingdom, Phylum, Class, Order, Family, Genus, Species), by = c("name" = "ASV"))
...
ggsave(file.path(output_dir, "Kansas_Regular_ITS_network.pdf"), p_ks_r, width = 8, height = 7)
write.csv(ks_r_edges, file.path(output_dir, "Kansas_Regular_ITS_edges.csv"), row.names = FALSE)
write.csv(as_tibble(as_data_frame(g_ks_r, what = "vertices")), file.path(output_dir, "Kansas_Regular_ITS_nodes.csv"), row.names = FALSE)
...
sd_e_nodes <- tibble(name = sort(unique(c(sd_e_edges$from, sd_e_edges$to)))) %>% left_join(tax %>% select(ASV, Kingdom, Phylum, Class, Order, Family, Genus, Species), by = c("name" = "ASV"))
...
ggsave(file.path(output_dir, "South_Dakota_Early_ITS_network.pdf"), p_sd_e, width = 8, height = 7)
write.csv(sd_e_edges, file.path(output_dir, "South_Dakota_Early_ITS_edges.csv"), row.names = FALSE)
write.csv(as_tibble(as_data_frame(g_sd_e, what = "vertices")), file.path(output_dir, "South_Dakota_Early_ITS_nodes.csv"), row.names = FALSE)
...
sd_r_nodes <- tibble(name = sort(unique(c(sd_r_edges$from, sd_r_edges$to)))) %>% left_join(tax %>% select(ASV, Kingdom, Phylum, Class, Order, Family, Genus, Species), by = c("name" = "ASV"))
...
ggsave(file.path(output_dir, "South_Dakota_Regular_ITS_network.pdf"), p_sd_r, width = 8, height = 7)
write.csv(sd_r_edges, file.path(output_dir, "South_Dakota_Regular_ITS_edges.csv"), row.names = FALSE)
write.csv(as_tibble(as_data_frame(g_sd_r, what = "vertices")), file.path(output_dir, "South_Dakota_Regular_ITS_nodes.csv"), row.names = FALSE)


#################################
##### COMBINED FOUR NETWORKS ####
#################################

fig_its_networks <- (p_ks_e | p_ks_r) / (p_sd_e | p_sd_r) + plot_annotation(title = "Fungal co-occurrence networks") & theme(plot.title = element_text(face = "bold", hjust = 0.5))

fig_its_networks

ggsave(file.path(output_dir, "All_four_ITS_networks.pdf"), fig_its_networks, width = 16, height = 13)
...
metrics_ks_e <- tibble(Network = "Kansas Early", Nodes = vcount(g_ks_e), Edges = ecount(g_ks_e), Average_degree = mean(degree(g_ks_e)), Density = edge_density(g_ks_e, loops = FALSE), Modularity = modularity(cluster_louvain(g_ks_e)), Clustering = transitivity(g_ks_e, type = "average", isolates = "zero"), Average_path_length = mean_distance(g_ks_e, directed = FALSE, unconnected = TRUE), Components = components(g_ks_e)$no, Positive_edges = sum(E(g_ks_e)$edge_sign == "Positive"), Negative_edges = sum(E(g_ks_e)$edge_sign == "Negative"), Percent_negative = 100 * sum(E(g_ks_e)$edge_sign == "Negative") / ecount(g_ks_e))

metrics_ks_r <- tibble(Network = "Kansas Regular", Nodes = vcount(g_ks_r), Edges = ecount(g_ks_r), Average_degree = mean(degree(g_ks_r)), Density = edge_density(g_ks_r, loops = FALSE), Modularity = modularity(cluster_louvain(g_ks_r)), Clustering = transitivity(g_ks_r, type = "average", isolates = "zero"), Average_path_length = mean_distance(g_ks_r, directed = FALSE, unconnected = TRUE), Components = components(g_ks_r)$no, Positive_edges = sum(E(g_ks_r)$edge_sign == "Positive"), Negative_edges = sum(E(g_ks_r)$edge_sign == "Negative"), Percent_negative = 100 * sum(E(g_ks_r)$edge_sign == "Negative") / ecount(g_ks_r))

metrics_sd_e <- tibble(Network = "South Dakota Early", Nodes = vcount(g_sd_e), Edges = ecount(g_sd_e), Average_degree = mean(degree(g_sd_e)), Density = edge_density(g_sd_e, loops = FALSE), Modularity = modularity(cluster_louvain(g_sd_e)), Clustering = transitivity(g_sd_e, type = "average", isolates = "zero"), Average_path_length = mean_distance(g_sd_e, directed = FALSE, unconnected = TRUE), Components = components(g_sd_e)$no, Positive_edges = sum(E(g_sd_e)$edge_sign == "Positive"), Negative_edges = sum(E(g_sd_e)$edge_sign == "Negative"), Percent_negative = 100 * sum(E(g_sd_e)$edge_sign == "Negative") / ecount(g_sd_e))

metrics_sd_r <- tibble(Network = "South Dakota Regular", Nodes = vcount(g_sd_r), Edges = ecount(g_sd_r), Average_degree = mean(degree(g_sd_r)), Density = edge_density(g_sd_r, loops = FALSE), Modularity = modularity(cluster_louvain(g_sd_r)), Clustering = transitivity(g_sd_r, type = "average", isolates = "zero"), Average_path_length = mean_distance(g_sd_r, directed = FALSE, unconnected = TRUE), Components = components(g_sd_r)$no, Positive_edges = sum(E(g_sd_r)$edge_sign == "Positive"), Negative_edges = sum(E(g_sd_r)$edge_sign == "Negative"), Percent_negative = 100 * sum(E(g_sd_r)$edge_sign == "Negative") / ecount(g_sd_r))

network_metrics <- bind_rows(metrics_ks_e, metrics_ks_r, metrics_sd_e, metrics_sd_r)

network_metrics

write.csv(network_metrics, file.path(output_dir, "ITS_All_network_metrics.csv"), row.names = FALSE)
...
write.csv(hub_ks_e, file.path(output_dir, "Kansas_Early_ITS_hub_ASVs.csv"), row.names = FALSE)
write.csv(hub_ks_r, file.path(output_dir, "Kansas_Regular_ITS_hub_ASVs.csv"), row.names = FALSE)
write.csv(hub_sd_e, file.path(output_dir, "South_Dakota_Early_ITS_hub_ASVs.csv"), row.names = FALSE)
write.csv(hub_sd_r, file.path(output_dir, "South_Dakota_Regular_ITS_hub_ASVs.csv"), row.names = FALSE)
...
write.csv(gatekeeper_ks_e, file.path(output_dir, "Kansas_Early_ITS_gatekeeper_ASVs.csv"), row.names = FALSE)
write.csv(gatekeeper_ks_r, file.path(output_dir, "Kansas_Regular_ITS_gatekeeper_ASVs.csv"), row.names = FALSE)
write.csv(gatekeeper_sd_e, file.path(output_dir, "South_Dakota_Early_ITS_gatekeeper_ASVs.csv"), row.names = FALSE)
write.csv(gatekeeper_sd_r, file.path(output_dir, "South_Dakota_Regular_ITS_gatekeeper_ASVs.csv"), row.names = FALSE)
